...
def get_specific_graph(courses, certificates, relevant_roles, info_level, starting_nodes, resource_type='both'):
    """
    ...
    Parameters:
        ...
        resource_type (str): 'course', 'certificate', or 'both'.
    ...
    sources = []
    for i in courses + certificates:
        if resource_type == 'course' and type(i).__name__ != 'Course':
            continue
        if resource_type == 'certificate' and type(i).__name__ != 'Certificate':
            continue
        if i.job_role == 'All':
            sources.append(i)
    ...
    for src in sources:
        if src.display in starting_nodes and src not in G:
            queue.append(src)
            G.add_node(src)
            if type(src).__name__ == 'Certificate':
                    ...
                    for i, src in enumerate(sources):
                        if node.is_prereq_to(src):
                            if src not in G:
                                G.add_node(src)
                                queue.append(src)
                                if type(src).__name__ == 'Certificate' and src.display not in starting_nodes:
